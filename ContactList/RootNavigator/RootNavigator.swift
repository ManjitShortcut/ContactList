...
final class RootNavigator {
    ...
    func start() {
        if let nav  = navigationController {
            if  let token =  KeyChainStroage.getToken(), token.count > 0 {
                AutheticationToken.sharedInstace.setAutheticationToken(string: token)
                let userList = self.getUserListViewController()
                navigationController?.setViewControllers([userList], animated: true)
            } else {
                // check in
